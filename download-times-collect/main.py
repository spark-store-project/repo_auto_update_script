...
# 读取程序分类列表
appList = []
for i in os.listdir(f"{packagePath}/store"):  # 遍历 deb 目录
    if os.path.isdir(f"{packagePath}/store/{i}"):  # 分类不可能是文件，是文件夹，所以需要加判断进行筛选
        appList.append(i)
...
class SPK:
    ...
    def save(self):
        for i in appList:
            ...
            folder = f"{self.savePath}/store/{i}/{self.package}/"
            # 文件不存在的情况
...
class Save(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        try:
            ...
            global bug
            bug += 1
            # 通过输出，可以方便定位程序第几次错误抛出的错误内容
            print(f"#########第{bug}个错误，不影响程序继续运行##########")
            traceback.print_exc()  # 输出真正的报错内容
